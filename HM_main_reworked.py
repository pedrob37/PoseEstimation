...
from HM_trainTT import HM_trainTT, testTT
...
if __name__ == '__main__':
    ...
    if not debug:
        ...
        # Create data dicts
        train_data_dict = [{'image': image_name, 'label': label_name, 'heatmap': heatmap_name} for
                           image_name, label_name, heatmap_name
                           in zip(train_images, train_labels, train_heatmaps.values())]

        val_data_dict = [{'image': image_name, 'label': label_name, 'heatmap': heatmap_name} for
                         image_name, label_name, heatmap_name
                         in zip(val_images, val_labels, val_heatmaps.values())]

        inf_data_dict = [{'image': image_name, 'label': label_name, 'heatmap': heatmap_name} for
                         image_name, label_name, heatmap_name
                         in zip(inf_images, inf_labels, inf_heatmaps.values())]
        ...
        # MONAI transforms
        relevant_keys = ['image', 'label'] + [f"heatmap_{hm}" for hm in selected_heatmaps]
        ...
        nearest_list = ["nearest"] * num_heatmaps
        zeros_list = ["zeros"] * num_heatmaps
    ...
    if opt.phase == "train":
        TT = HM_trainTT(MODELS_DIR, FIG_DIR, writer, train_loader, val_loader, opt, model, optimizer, scheduler,
                     criterion_heatmap,
                     selected_heatmaps, current_epoch=current_epoch, current_iteration=current_iteration,
                     debug=opt.debug)
        TT.train()
